...
ECV protects the meaning of its bounded verdict: a caller should not receive `EXACTLY_VERIFIED_IN_DOMAIN` unless the exact structured predicate in the document's selected `ECV/1` or `ECV/2` contract recomputes true.

The trusted computing base for v0.2 is:
...
No network service, model, solver binary, plugin, private runtime, or private registry is in the runtime TCB.
...
- false linear-system, polynomial, CRT, or LP-certificate claims;
...
Within the selected numbered ECV contract and the stated TCB:
...
- LP optimality is accepted only when exact primal and dual feasibility and objective equality all close;
...
- a malicious or defective independent implementation merely because it passes the finite conformance corpus.
...
A Verifiable Evidence Capsule can bind ECV input/result bytes to a content-addressed inventory and an externally pinned root. The executable `VEC_ECV_PROFILE_V1` described by VEC verifies those bindings and replays the checker command, but does not prove that the formal claim matches a natural-language statement or real-world fact. Keep byte-integrity and mathematical-verdict claims distinct.
